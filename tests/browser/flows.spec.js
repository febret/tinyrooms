...
test.describe("account onboarding", () => {
  ...
  test("supports designing a custom sticker during onboarding", { tag: "@mobile" }, async ({ page, runtime }) => {
    ...
    await frame.locator('[data-category="eyes"]').click();
    await frame.locator('[data-part="eyes-starry"]').click();
    await frame.locator('[data-category="mouth"]').click();
    await frame.locator('[data-part="mouth-grin"]').click();
    ...
    expect(user.sticker_design.eyes).toBe("eyes-starry");
    expect(user.sticker_design.mouth).toBe("mouth-grin");
...
test.describe("milestone 3 room editing", () => {
  ...
  test("editor adds, transforms, snaps, and undoes a decorative prop", async ({ page, runtime, isMobile }) => {
    ...
    await createEditorAccount(page, runtime, "editor");
    const panel = await openEditRoom(page);
    // The base propset puts the shared decorative catalog in every library.
    expect(await panel.locator(".editor-library-item").count()).toBeGreaterThan(1);
